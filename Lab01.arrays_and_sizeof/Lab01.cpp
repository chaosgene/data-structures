	...
	// Find the maximum integer in an array
		if (arraySize > 0) {
			...
			for (int i = 1; i < arraySize; i++) {
                // Check ea. element if greater than the current running maximum
				if (array[i] > max) {
			...
			retV = -10000; // If maximum cannot be found, set specified value
		}
	}
	return retV;
}
...
int Lab01::question1b (int *A, int arraySize) {
	...
	// Find the maximum integer in an array
		if (arraySize > 0) {
            ...
            // If a maximum cannot be found, set the specified value
			retV = -10000;
 		}
	}
	return retV;
...
int Lab01::question3(int first, int second) {
	...
	lcm = first;
	if ((first > 0) && (second > 0)) {
        // Check if multiples of one input are divisible by the other
		for (int c = 2; (lcm % second) != 0; c++) {
			lcm = first * c;
		}
	} else {
		lcm = -1;
...
void Lab01::question6() {
	...
	// Since the upper-left corner is an X and this is a multidimensional array
    // of ints, separately print values
	cout << "X" << "\t";
...
void Lab01::sizeofUse() {
	...
	It tell us how many bytes of memory have been allocated to store something
    of this type. 
	...
	Stack-allocated arrays take up as many bytes as they have items times the
    space needed for each item.
	Therefore, sizeof is a very low-level indicator of how much memory is being
    used.
	Extra calculations will be necessary to find the actual number of elements
    in an array, for example (knowing the type and amount of bytes one item of
    such type needs is a start). 
	*/
