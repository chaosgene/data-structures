...
/*
	Assignment Operator
*/
StatisticsProcessor& StatisticsProcessor::operator=(const StatisticsProcessor
        &rhs) {
...
void StatisticsProcessor::loadData(std::string file_name) {
	...
	if (myfilestream.is_open()) {
		...
		int numbLines;
		istringstream ( line ) >> numbLines;
        // now contains the num of lines to be read from the file for input
		int currentEntry;
...
istream& operator>>(istream &in, StatisticsProcessor &s) {
	...
	for (int i = 0; i < numbEntries; i++) {
		...
		while (!f2) {
			bool t2 = true; // t means if need to check again after this one
...
*/
void StatisticsProcessor::writeData(string file_name) {
	ofstream myfile;
